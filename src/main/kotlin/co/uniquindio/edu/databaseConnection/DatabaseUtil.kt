...
class DatabaseUtil {
    ...
    fun initDatabaseAndTables(){
        ...
        createTable("CREATE TABLE IF NOT EXISTS Secretary(code VARCHAR(11) NOT NULL, name VARCHAR(255), last_name VARCHAR(255), phone_number VARCHAR(255), email VARCHAR(255), PRIMARY KEY(code));")
        createTable("CREATE TABLE IF NOT EXISTS Trainer(code VARCHAR(11) NOT NULL, name VARCHAR(255), last_name VARCHAR(255), phone_number VARCHAR(255), email VARCHAR(255), PRIMARY KEY(code));")
        createTable("CREATE TABLE IF NOT EXISTS PhysicalAssessment(code int NOT NULL AUTO_INCREMENT, date DATE, arms DECIMAL, legs DECIMAL, hips DECIMAL, height DECIMAL, weight DECIMAL, personal_goals LONGTEXT, code_trainer VARCHAR(11) NOT NULL, PRIMARY KEY(code), CONSTRAINT fk_code_trainer FOREIGN KEY(code_trainer) REFERENCES Trainer(code) ON UPDATE CASCADE);")
