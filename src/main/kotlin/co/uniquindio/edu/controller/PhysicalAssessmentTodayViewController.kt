...
class PhysicalAssessmentTodayViewController {
    @FXML
    ...
    fun handleUpdateButton(e: ActionEvent){
        if(physicalAssessmentTableView.selectionModel.selectedItem != null) {
            if (isInputValid()) {
                ...
                adminEJB.updatePhysicalAssessment(itemSelected.code.toInt(), heightField.text.toDouble(), weightField.text.toDouble(), armsField.text.toDouble(), legsField.text.toDouble(), hipsField.text.toDouble(), personalGoalsArea.text, trainer.code)
    ...
    private fun isInputValid():Boolean{
        ...
        if(personalGoalsArea.text.isNullOrEmpty()){
            errorMessage += "Debes ingresar las metas personales del miembro\n"
        }
